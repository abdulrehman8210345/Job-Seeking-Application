...
export const employerGetallapplications = catchAsyncError(
  async (req, res, next) => {
    ...
    if (role === "Job Seeker") {
      return next(
        errorObject("Job Seeker is not allowed for this resource", 400)
      );
    }
...
export const jobSeekerGetallapplications = catchAsyncError(
  async (req, res, next) => {
    ...
    if (role === "Employer") {
      return next(
        errorObject("Employer is not allowed for this resource", 400)
      );
    }
...
export const postapplication = catchAsyncError(async (req, res, next) => {
  ...
  if (!formatsAllowed.includes(resume.mimetype)) {
    return next(
      errorObject(
        "Invalid file format!!,Please upload image in jpg/jpeg/png/webp format",
        400
      )
    );
  }
  ...
  if (
    ...
    return next(
      errorObject("Please fill out all required application details", 400)
    );
  }
